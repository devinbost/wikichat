...
async function queryCQLDatabase(email: string) {
    try {
        ...
        if (result.rows.length > 0) {
            ...
            console.error("No user found with the provided email in DB");
            return null;
        }
    } catch (error) {
...
export async function POST(request: Request) {
    ...
    //         return response;
    //     } else {
    //         console.error("Invalid credentials for known user in login flow");
    //         return NextResponse.json({ message: "Invalid credentials" }, { status: 401 });
    //     }
    // } else {
    //     console.error("Invalid credentials in login flow");
    //     return NextResponse.json({ message: "Invalid credentials" }, { status: 401 });
    // }
}
